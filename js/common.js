var common = function () {
	var com = {
			...
			keygen =
				...
				function () {
					...
					for (var i = 0; i < arguments.length; i++) {
						out += com.constimap[com.ctype(arguments[i])]
							.substring(0, 1)
							.toLowerCase();
					}
					return out;
		...
		randBetween: function (a_min, a_max, a_int) {
			...
			if (a_min === a_max) {
				...
				if (asInteger) {
					...
					return (
						parseFloat(a_min) +
						(parseFloat(a_max) - parseFloat(a_min)) * Math.random()
					);
				}
			}
		},
		...
		floorcolor: function (x, divider, offset, alpha) {
			...
			return (
				"rgba(" + redpart + "," + bluepart + "," + greenpart + "," + alpha + ")"
			);
		},
		sesmix: function (x, incoef, inminus, finalcoef, finalminus) {
			incoef = incoef || [1, 1, 1];
			finalcoef = finalcoef || [0.33, 0.33, 0.33];
			...
			var rr =
				255 *
				finalcoef.reduce(function (acc, item, index) {
		...
		randomRGBA: function () {
			...
			return (
				"rgba(" +
				com.randBetween(p[0], p[1], true) +
				"," +
				com.randBetween(p[2], p[3], true) +
				"," +
				com.randBetween(p[4], p[5], true) +
				"," +
				com.randBetween(p[6], p[7], false) +
				")"
			);
		},
		...
		supercollidereasyrep: function (str) {
			...
			var output = "arg ";
			output += Object.keys(repobj.dict)
		...
		*/
		...
		normalizeArray: function (a_arr, a_ranges) {
			...
			var out = a_arr.map(function (a) {
				return (
					outRanges[0] +
					((a - minmax[0]) / rangeSize) * (outRanges[1] - outRanges[0])
				);
			});
			return out;
		},
			...
			for (var i in a_obj) {
				...
				for (var j in a_obj[i]) {
					if (a_filter(a_obj[i][j])) {
						out[i][j] =
							a_ranges[0] +
							((a_obj[i][j] - rng[0]) / (rng[1] - rng[0])) *
								(a_ranges[1] - a_ranges[0]);
					}
				}
			}
			return out;
		...
		arrayToObj: function (
			...
			for (var i = 0; i < a_arr.length; i++) {
				if (outObj[a_arr[i][a_primaryIndex]]) {
					outObj[a_arr[i][a_primaryIndex]][a_arr[i][a_secondaryIndex]] =
						a_valfn(a_arr[i][a_valueIndex]);
				} else {
					var o = {};
		...
		loadCorpus: function (a_key, a_keyword, a_fn) {
			$.get(
				"http://computerscare.com/schoolchildren/rf.php?full=" + a_keyword,
				{},
				function (data) {
		...
		rf: function (query, fn) {
			$.post(
				"http://computerscare.com/schoolchildren/rf.php",
				{
					full: query,
				},
				fn,
		...
		addcorpus: function (query, num, callback) {
			...
			num = num || 3;
			$.get(
				"http://computerscare.com/schoolchildren/rf.php?full=" +
					query +
					"&fnum=" +
					num,
				{},
		...
		loadCorpusCommands: function (options, callback) {
			$.get(
				"http://computerscare.com/schoolchildren/categoryList.php",
				options || {},
				function (data) {
					if (callback) {
		...
		loadAllCorpus: function (a_num, callback) {
			$.get(
				"http://computerscare.com/schoolchildren/getcorpus.php",
				{
					num: a_num || 20,
				},
				function (data) {
		...
		windowUtils: function (outerSelector, innerSelector) {
			var windowStuff = {
				...
				resized: function () {
					...
					windowStuff.pixelRatio =
						"devicePixelRatio" in window ? window.devicePixelRatio : 1;
					windowStuff.scaledWidth =
						windowStuff.pixelRatio * windowStuff.outerwidth;
					windowStuff.scaledHeight =
						windowStuff.pixelRatio * windowStuff.outerheight;
				},
			};
			return windowStuff;
		},
		...
		autoarg: function (args) {
			var type,
				dex,
				rg = Array.prototype.slice.call(args, 0);
			var out = [[], [], [], [], [], [], [], []];
		...
		reMap: function (a) {
			...
			if (mtch) {
				...
				if (city) {
					...
					$("#output").css(
						city[0],
						parseInt($("#output").css(city[0]), 10) + 1,
					);
				}
			}
		},
		...
		qm: {
			...
			vrot: function (vec, theta, unit) {
				...
				var cross = [
					unit[1] * vec[2] - unit[2] * vec[1],
					unit[2] * vec[0] - unit[0] * vec[2],
					unit[0] * vec[1] - unit[1] * vec[0],
				];
		...
		num: {
			...
			ep: function (x, y, cx, cy, cr) {
				return Math.exp(-com.num.len(x, y, cx, cy) / (cr || 0.0000001));
			},
			...
			sinest: function (x, omega, phi, amp, fn) {
				...
				for (var i = Math.max(omega.length, phi.length) - 1; i >= 0; i--) {
					out = fn.call(
						null,
						x * omega[i % omega.length] -
							phi[i % phi.length] +
							amp[i % amp.length] * out,
						i,
						omega,
						phi,
						out,
			...
			sinsum: function (x, omega, phi, amp, fn) {
				...
				for (
					var i = 0;
					i < Math.max(omega.length, phi.length, amp.length);
					i++
				) {
					out +=
						amp[i % amp.length] *
		...
		complex: {
			...
			div: function (a, b) {
				...
				return [
					...
					(b[0] * a[1] - b[1] * a[0]) / modulus,
				];
			},
			sin: function (a) {
				return [
					Math.cosh(a[0]) * Math.cos(a[1]),
					Math.sinh(a[0]) * Math.sin(a[1]),
				];
			},
			cos: function (a) {
				return [
					Math.sinh(a[0]) * Math.cos(a[1]),
					Math.cosh(a[0]) * Math.sin(a[1]),
				];
			},
			conj: function (a) {
				return [a[0], -a[1]];
		...
		arr: {
			...
			tensorInv: function (digit, arr) {
				return arr.map(function (item) {
					var out = [];
					var len = Math.max(
						item[0].toString().length,
						item[1].toString().length,
					);
					var modder = Math.pow(10, len);
					var carry =
						((item[0] * item[1] - ((item[0] * item[1]) % modder)) / modder) %
						10;
				...
				for (var i = 0; i < n; i++) {
					out.push(
						M *
							(1 -
								Math.cos(
									omega[i % omega.length] * x + angle * i + phi[i % phi.length],
								)),
					);
				}
				return out;
			...
			reparg: function (str, re, fn) {
				...
				return function () {
					...
					if (innerargs.length === 0) {
						innerargs = "abcdefghijklmnopqrstuvwxyz"
							.split("")
							.map(function (item) {
								return item + "_" + item;
							});
	...
	com.parse = (function () {
		...
		function done() {
			...
			allfn = allfn.concat(
				infix.map(function (item) {
					return {
						numArg: 2,
						val: item,
						fn: function (lhs, rhs, val) {
							return (
								"(" + (lhs || def1) + ")" + val + "(" + (rhs || def2) + ")"
							);
						},
					};
				}),
			...
			allfn = allfn.concat(
				com.js.jsmath2.map(function (item) {
					return {
						numArg: 2,
						val: item,
					};
			...
			allfn = allfn.concat(
				com.js.jsmathn.map(function (item) {
					return {
						numArg: 2,
						val: item,
					};
			...
			allfn = allfn.concat(
				com.js.jsmath1.map(function (item) {
					return {
						numArg: 1,
						val: item,
					};
	...
	com.grid = (function () {
		var grid = com.typemapper(),
			...
			h,
			wouter = com.wrapper(
				'<div class="comGrid" style="position:relative;">',
				"</div>",
			),
			winner,
		...
		function gridHTML(arr) {
			...
			for (var j = 0; j < ny; j++) {
				for (var i = 0; i < nx; i++) {
					var stylepart =
						'style="position:absolute; text-align:center; width:' +
						w +
						"px; height:" +
						h +
						"px; " +
					...
					nn = j * nx + i;
					winner = com.wrapper(
						'<div class="gridBox" ' + stylepart + 'data-nn="' + nn + '">',
						"</div>",
					);
					html += winner(
						arr[nn] === undefined ? "" : gridTextFn(arr[nn], nn, arr),
					);
				}
			}
			return wouter(html);
	...
	com.hrid = (function () {
		var hrid = com.typemapper(),
			map = {
				...
				b: function (num) {
					var width = (1 / num) * 100 + "%";
					return (
						"float:left;position:relative;height:100%;width:" + width + ";"
					);
				},
			};
		function rapper(type, cl, num) {
			...
			return function (str) {
				str = str || "(" + type + "," + cl + ")";
				return (
					'<div class="' + cl + '" + style="' + sty + '">' + str + "</div>"
				);
			};
		}
		/*===========================================================================
	...
	com.betterkeyboard = (function () {
		...
		bk.add("as", function (a, s) {
			return bk.go(a, function () {}, s);
		});
		bk.add("a", function (a) {
			return bk.go.call(null, a, function () {}, "coldt");
		});
		return bk.make();
	})();
	com.fnmake = (function () {
